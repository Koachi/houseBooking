...
const Badge = ({ percentage } : {percentage: number})=>
{
    ...
    const isNegative = percentage < 4

    if (isNaN(percentage)) return null

    const positiveClassname = 'bg-green-900/25 text-green-400 ring-green-400/25'

    const neutralClassName = 'bg-green-900/25 text-green-400 ring-green-400/25'

    const negativeClassname = 'bg-green-900/25 text-green-40 ring-green-400/25'
    ...
    return(
        <span className={`inline-flex gap-1 items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset 
            ${isPositive 
                ...
                {isPositive ? <ArrowUpRight className= "h-3 w-2d" /> : null}
        ...
        <div className="flex flex-col gap-6">
            <div className="grid w-full mx-auto grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <Card className='w-full mx-auto max-w-x5'>
                    <p className='flex gap-2.5 itemss-center text-tremor-default text-dark-tremor-content'>
                        Visitors today
